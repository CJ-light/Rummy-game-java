...
public class Hand implements HandInterface {
   ...
   protected java.util.List<Card> hand = new ArrayList<Card>();
   ...
   */
   public Card [] findSet( ){
    HashMap <String, ArrayList<Card>> map = new HashMap<String,ArrayList<Card>>();
      String firstSetRank = null; //-1 means that it never found a set, any other number means that that was the first set
      for (Card card:hand){
        String rank = Character.toString(card.getRank());
        if (map.get(rank) == null){
          ArrayList<Card> temp = new ArrayList<Card>();
          map.put(rank, temp);
        }
        map.get(rank).add(card);
        if (firstSetRank == null){
          if (map.get(rank).size() >= 3){
            firstSetRank = rank;
          }
        }
        else if (map.get(rank).size() == 4){
          Card set[] = new Card[4];
          for (int i = 0; i < 4; i++){
            set[i] = map.get(firstSetRank).get(i);
          }
          return set;
        }
      }
   ...
   */
    ...
    */
    public String toString() {
        ...
        return handstring.substring(1,handstring.length()-1);
    }
